...
export const handler = async (event) => {
  const {email, password, userType , longitude, latitude, trashTypes, companyName } = JSON.parse(event.body);
  try {
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ accessToken: AccessToken, refreshToken: RefreshToken, idToken: IdToken, userType: "Consumer" }),
    };
  } catch (error) {
